...
export async function markSignatureUsed(signature, kvUrl, kvToken) {
    ...
    const ttlSeconds = Math.ceil(MAX_MESSAGE_AGE / 1000) * 2; // 2x message age for safety
...
export async function countActiveOffers(wallet, kvUrl, kvToken) {
    ...
    try {
        ...
        return MAX_ACTIVE_OFFERS_PER_WALLET;
    }
}
...
export async function kvSetEx(key, seconds, value, kvUrl, kvToken) {
    const res = await fetch(`${kvUrl}/setex/${key}/${seconds}/${encodeURIComponent(typeof value === 'string' ? value : JSON.stringify(value))}`, {
        headers: { 'Authorization': `Bearer ${kvToken}` }
    });
    if (!res.ok) {
        console.error(`KV SETEX failed for ${key}: ${res.status} ${res.statusText}`);
    }
}

export async function kvTtl(key, kvUrl, kvToken) {
    const res = await fetch(`${kvUrl}/ttl/${key}`, {
        headers: { 'Authorization': `Bearer ${kvToken}` }
    });
    const data = await res.json();
    return data.result;
...
export async function kvIncr(key, kvUrl, kvToken) {
...
// ========== Hash Operations (for per-field atomic access) ==========

export async function kvHset(key, field, value, kvUrl, kvToken) {
    const res = await fetch(kvUrl, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${kvToken}`, 'Content-Type': 'application/json' },
...
export async function kvHget(key, field, kvUrl, kvToken) {
    const res = await fetch(kvUrl, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${kvToken}`, 'Content-Type': 'application/json' },
...
export async function kvHdel(key, field, kvUrl, kvToken) {
    const res = await fetch(kvUrl, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${kvToken}`, 'Content-Type': 'application/json' },
...
export async function kvHgetall(key, kvUrl, kvToken) {
    const res = await fetch(kvUrl, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${kvToken}`, 'Content-Type': 'application/json' },
    ...
    for (let i = 0; i < data.result.length; i += 2) {
        try {
            map[data.result[i]] = typeof data.result[i + 1] === 'string'
                ? JSON.parse(data.result[i + 1])
                : data.result[i + 1];
        } catch {
            map[data.result[i]] = data.result[i + 1];
        }
    }
    return map;
}
...
/**
 * Migrate a key from string (JSON blob) to Redis hash format.
 * Safe to call multiple times — skips if already migrated or empty.
 */
export async function migrateMapToHash(key, kvUrl, kvToken) {
    try {
        const raw = await kvGet(key, kvUrl, kvToken);
        if (!raw || typeof raw !== 'object' || Object.keys(raw).length === 0) return;

        // Old string format found — migrate to hash under new key
        const hashKey = `${key}:h`;
        for (const [field, value] of Object.entries(raw)) {
            await kvHset(hashKey, field, value, kvUrl, kvToken);
        }
        // Delete old string key
        await kvDelete(key, kvUrl, kvToken);
        console.log(`Migrated ${key} → ${hashKey} (${Object.keys(raw).length} entries)`);
    } catch (e) {
        // Already migrated or WRONGTYPE — safe to ignore
    }
}
...
export async function executeEscrowTransaction(transaction, escrowKeypair, heliusApiKey, maxRetries = 2) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        ...
        // Clear previous signatures so we can re-sign with a fresh blockhash
        ...
        try {
            ...
            if (isBlockhashError && attempt < maxRetries) {
                console.warn(`Blockhash expired on attempt ${attempt + 1}, retrying with fresh blockhash...`);
                continue;
            }
            throw err;
        }
